...
export default function Staff() {
  ...
  const handleToggleStatus = async (user) => {
    try {
      await dispatch(toggleStaffStatus(user._id)).unwrap();
      showToast.success(`Staff ${user.isActive ? 'deactivated' : 'activated'} successfully!`);
    } catch (error) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
        ...
        <div className="flex gap-3">
          ...
          {/* Add New Button - Now navigates to separate page */}
          <button
            onClick={handleAddStaff}
            className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3.5 rounded-2xl font-black uppercase tracking-widest transition-all shadow-lg shadow-blue-500/25 flex items-center gap-2"
          >
            <UserPlus size={20} />
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-slate-400 font-black text-xl">No Staff Members</p>
            <p className="text-slate-300 mt-2">Add your first staff member</p>
            <button
              onClick={handleAddStaff}
              className="mt-6 bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-blue-500/30 transition-all inline-flex items-center gap-2"
            >
              <PlusCircle size={20} />
              Add Staff
            </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="p-6">
              ...
              <h2 className="text-2xl font-black text-center text-slate-800 mb-2">Delete Staff</h2>
